...
class FloatTextWatcher(val view: TextView, val submit: Button?): TextWatcher {
    ...
    override fun onTextChanged(p0: CharSequence?, p1: Int, p2: Int, p3: Int) {
        if (view.isFocused){
            if (p0.isNullOrBlank()){
                ...
                if (p0.matches(Regex("([0-9])+(\\.([0-9])+)?"))) {
                    view.error = null
                } else {
                    view.error = "El campo debe ser un float separado con punto"
                    if (submit != null) {
